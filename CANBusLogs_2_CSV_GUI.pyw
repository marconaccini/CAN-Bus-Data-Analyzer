...
class CANBusLogs2CSVGUI(tk.Tk):
    def __init__(self):
        ...
        # disabled-> self.info_row = tk.BooleanVar(value=False)
        self.unit_row = tk.BooleanVar(value=True)
        self.extract_values = tk.BooleanVar(value=True) # Nuova opzione, attiva di default
    ...
    def create_widgets(self):
        ...
        opts_frame2 = ttk.Frame(self)
        opts_frame2.pack(fill="x", padx=padx, pady=pady)
        # disabled-> ttk.Checkbutton(opts_frame2, text="Insert info data", variable=self.info_row).pack(side="left", padx=2)
        ttk.Checkbutton(opts_frame2, text="Insert units", variable=self.unit_row).pack(side="left", padx=12)
        ttk.Checkbutton(opts_frame2, text="Decode VAL_ values (_<name>_values)", variable=self.extract_values).pack(side="left", padx=12)
    ...
    def run_conversion(self):
        ...
        try:
            ...
            # disabled-> if self.info_row.get():
            # disabled->     args.append("--info")
            if self.unit_row.get():
                args.append("--units")

            # nuovo argomento se la checkbox è selezionata
            if self.extract_values.get():
                args.append("--extract_values") # O -ev, a seconda di come l'hai chiamato nel parser
